...
extension MeetingViewController: PubSubMessageListener {
    ...
    func onMessageReceived(_ message: PubSubMessage) {
        ...
        if(message.topic == RAISE_HAND_TOPIC){
            let localParticipantName = participants.first(where: { $0.isLocal == true })?.displayName
            self.showToast(message: "\(message.senderName == localParticipantName ? "You" : "\(message.senderName)") raised hand 🖐🏼", font: .systemFont(ofSize: 18))
        } else {
        ...
        buttonControlsView.onMenuButtonTapped = {
            ...
            self.showActionsheet(options: menuOptions, fromView: self.buttonControlsView.menuButton) { option in
                ...
                case .showParticipantList:
                    ...
                    let newViewController = storyBoard.instantiateViewController(withIdentifier: "ParticipantsViewController") as! ParticipantsViewController
                    newViewController.participants = self.participants
